...
def main(args=None):
    ...
    try:
        ...
        # msg.vel_des = [0, 0.1, 0] 
        # msg.duration = 1500 
        # msg.step_height = [0.15, 0.14] # ground clearness of swing leg
        # msg.life_count += 1
        # Ctrl.Send_cmd(msg)
        # Ctrl.Wait_finish(11, 3)
    ...
    #     Ctrl.fixAngle(msg)
    #     Ctrl.navigate(msg) #跳两次相当于翻转180°，之后回正
    #     Ctrl.fixAngle(msg)
    ...
    #     Ctrl.fixAngle(msg)
        ...
        while rclpy.ok():
            ...
            if sensor_subscriber.green_cx is not None and sensor_subscriber.red_cx is not None:
                print("green_cx{}".format(sensor_subscriber.green_cx))
                print("red_cx{}".format(sensor_subscriber.red_cx))
...
class Robot_Ctrl(object):
    ...
    def Wait_finish_middle(self, mode, gait_id):
        count = 0
        while self.runing and count < 1000: #10s
            if self.mode_ok == mode and self.gait_ok == gait_id:
                return True
            else:
                time.sleep(0.005)
